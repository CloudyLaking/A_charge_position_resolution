...
def main(le,amount,n,a,b,c,m,step,T0,v):
    ...
    for i in range(600000):
        ...
        if (i+1)%n==0:
            ...
            #开始收敛
            if diff1 < threshold*2 and diff2 < threshold*4 and diff1 > 0 and diff2 > 0:
                T = T * 0.93
                print(f'Converging... T={T}')
            if (diff1 < threshold and diff2 < threshold and diff1 > 0 and diff2 > 0) or T<0.000012:
                print('potential:{}\n'.format(pmin))
                break
    return data0
